...
if ! pacman -Qi autofs > /dev/null; then yes | sudo pacman -S autofs; fi # Automount
...
if ! systemctl is-active --quiet vmware-vmblock-fuse; then # Enable vmware services
	sudo systemctl enable vmware-vmblock-fuse
	sudo systemctl start vmware-vmblock-fuse
fi

if ! systemctl is-enabled --quiet autofs; then # Enable autofs services
	sudo systemctl enable autofs
fi

if ! [ -f /usr/share/fonts/noto/NotoColorEmoji.ttf ]; then # Apple Emoji Font
	git clone https://github.com/sethfl/linux-apple-emojis.git ~/linux-apple-emojis
	cd ~/linux-apple-emojis
	sudo make install
	yes | rm -fr ~/linux-apple-emojis
fi
