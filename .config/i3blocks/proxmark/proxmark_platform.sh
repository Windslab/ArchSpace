...
if ! [ -f ~/proxmark3/Makefile.platform ]; then sh -c 'printf "PLATFORM=PM3OTHER\n#PLATFORM_EXTRAS=BTADDON\n#STANDALONE=HF_MSDSAL\n" > ~/proxmark3/Makefile.platform'; fi

PL=$(grep 'PLATFORM=PM3*' ~/proxmark3/Makefile.platform | cut -d'=' -f 2)

if [[ $PL == "PM3OTHER" ]]; then
   sed -i "s/${PL}/PM3RDV4/g" ~/proxmark3/Makefile.platform
   sed -i "s/OTHR/RDV4/g" ~/.config/i3blocks/config
   i3-msg restart
else
   sed -i "s/PM3RDV4/PM3OTHER/g" ~/proxmark3/Makefile.platform
   sed -i "s/RDV4/OTHR/g" ~/.config/i3blocks/config
   i3-msg restart
fi
